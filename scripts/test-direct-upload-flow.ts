...
/**
 * Test du flux d'upload direct (sans modal intermédiaire)
 */
...
async function testDirectUploadFlow() {
  console.log('🧪 Test du flux d\'upload direct...\n');
  ...
  try {
    ...
    // 6. Vérifier que LeaseDocumentUploadModal a été supprimée
    console.log('\n🗑️ Vérification de la suppression de la modal intermédiaire...');
    try {
      await import('../src/components/leases/LeaseDocumentUploadModal.tsx');
      console.log('   ❌ LeaseDocumentUploadModal existe encore (ne devrait pas)');
    } catch (error) {
      console.log('   ✅ LeaseDocumentUploadModal supprimée avec succès');
    }

    // 7. Test de l'API de finalisation
    ...
    console.log('   ✅ 1️⃣ Modal intermédiaire supprimée: Plus de double modal');
    console.log('   ✅ 2️⃣ Flux d\'upload direct: Clic → Sélection fichier → UploadReviewModal');
    console.log('   ✅ 3️⃣ Liaisons prévues dans UploadReviewModal: Affichage des entités liées');
    console.log('   ✅ 4️⃣ Type de document verrouillé: Non modifiable quand pré-rempli');
    console.log('   ✅ 5️⃣ Avancement du workflow: Statut du bail mis à jour automatiquement');
    console.log('   ✅ 6️⃣ Cohérence avec le workflow existant: Même pattern que bien/baux');

    console.log('\n🎉 Flux d\'upload direct et optimisé !');
    ...
    console.log('   - Sélectionner le fichier directement');
    console.log('   - La modal de revue s\'ouvre avec le type verrouillé et les liaisons prévues');
