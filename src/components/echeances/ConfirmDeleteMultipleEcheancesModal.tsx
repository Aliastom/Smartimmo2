...
export function ConfirmDeleteMultipleEcheancesModal({
  ...
  return (
    <Modal
      ...
      title={actionType === 'delete' ? `Supprimer ${echeanceIds.length} échéance(s) ?` : `Désactiver ${echeanceIds.length} échéance(s) ?`}
      size="md"
        ...
        <>
          ...
          >
            {isProcessing ? (
              ...
              actionType === 'delete' ? `Supprimer ${echeanceIds.length} échéance(s)` : `Désactiver ${echeanceIds.length} échéance(s)`
            )}
          </Button>
        </>
      }
      ...
      <div className="space-y-4">
        ...
        <div className="text-center">
          <p className="text-sm text-gray-600">
            <span className="font-medium text-gray-900">{echeanceIds.length}</span> échéance(s) sélectionnée(s)
          </p>
        </div>
          ...
          <div className="flex items-start gap-3 p-4 bg-yellow-50 border border-yellow-200 rounded-lg">
            ...
            <div className="flex-1 text-sm">
              <p className="font-medium text-yellow-900 mb-1">
                Ces échéances seront désactivées et ne généreront plus de projections à partir d'aujourd'hui.
              </p>
          ...
          <div className="flex items-start gap-3 p-4 bg-red-50 border border-red-200 rounded-lg">
            ...
            <div className="flex-1 text-sm">
              <p className="font-medium text-red-900 mb-1">
                Cette action est IRRÉVERSIBLE et supprimera {echeanceIds.length} échéance(s) de la base de données.
              </p>
          ...
          <label className="flex items-start p-3 border-2 rounded-lg cursor-pointer transition-colors hover:bg-gray-50">
            ...
            <div className="ml-3 flex-1">
              <span className="text-sm font-medium text-gray-900">
                Désactiver les échéances
              </span>
              <p className="text-xs text-gray-500 mt-1">
                Les échéances seront désactivées mais conservées dans la base de données
              </p>
            </div>
          </label>
          ...
          <label className="flex items-start p-3 border-2 rounded-lg cursor-pointer transition-colors hover:bg-gray-50">
            ...
            <div className="ml-3 flex-1">
              ...
              <p className="text-xs text-red-600 mt-1">
                <strong>Action irréversible</strong> : les échéances seront définitivement supprimées
              </p>
            </div>
          </label>
        </div>
