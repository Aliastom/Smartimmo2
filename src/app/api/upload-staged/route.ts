...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log('[API] POST /api/upload-staged - Début');
    ...
    console.log('[API] POST /api/upload-staged - Données reçues:', {
      fileName: file?.name,
      fileSize: file?.size,
      uploadSessionId,
      typeId,
    ...
    if (!file || !uploadSessionId) {
      console.log('[API] POST /api/upload-staged - Erreur: Fichier ou session manquant');
    ...
    if (process.env.STORAGE_TYPE === 'supabase') {
      try {
        ...
        console.log('[API] POST /api/upload-staged - Fichier temporaire stocké dans Supabase Storage:', tempKey);
      } catch (uploadError: any) {
        console.error('[API] POST /api/upload-staged - Erreur upload vers Supabase Storage:', uploadError);
      ...
      console.log('[API] POST /api/upload-staged - Fichier temporaire stocké en local:', filePath);
    }
    ...
    console.error('[API] POST /api/upload-staged - Erreur:', error);
    console.error('[API] POST /api/upload-staged - Stack:', error instanceof Error ? error.stack : 'No stack');
    return NextResponse.json(
